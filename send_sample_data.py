...
# 로컬 개발 서버 API 엔드포인트 URL
API_URL = "http://127.0.0.1:8000/api/working_lists"
...
def generate_random_log():
    ...
    # Python Dictionary 포맷 데이터 조립 (has_error: 1 고정)
    dict_data = {
        "has_error": 1,
...
def send_data(worker_name, log_line):
    ...
    try:
        ...
        log_message("ERROR - API 서버 연결 실패 (http://127.0.0.1:8000)")
        if is_simulating:
            toggle_simulation()
...
root = tk.Tk()
root.title("AR Glass - Process Logs Transmission Tool (v1.0)")
...
lbl_title = tk.Label(frame_header, text="AR GLASS DATA MONITOR & SENDER", font=("바탕체", 12, "bold"), fg="#FFFFFF", bg="#1E293B")
lbl_title.pack()
...
# 콘솔 초기 가이드
log_message("INFO - 시스템 시뮬레이터 초기화 완료.")
log_message("INFO - 대시보드 주소: http://127.0.0.1:8000")
