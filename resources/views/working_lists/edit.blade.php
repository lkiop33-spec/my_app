            ...
            <div class="bg-gray-800 border border-gray-700/60 shadow-2xl rounded-none overflow-hidden p-8">
                <div class="mb-6 border-b border-gray-700 pb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">수정할 내용을 기입해 주세요.</p>
                    </div>
                    <span class="text-xs font-mono bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-3 py-1 rounded-lg">
                        ID: #{{ $workingList->id }}
                    </span>
                </div>

                <form method="POST" action="{{ route('working_lists.update', $workingList) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
