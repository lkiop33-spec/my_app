                ...
                <div class="p-6 text-white dark:text-gray-100">
                    <form method="POST" action="{{ isset($notice) ? route('notices.update', $notice) : route('notices.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="content" name="content" rows="10" class="block mt-1 w-full border-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>{{ old('content', $notice->content ?? '') }}</textarea>
                            <x-input-error :messages="$errors->get('content')" class="mt-2" />
                        </div>

                        <div class="flex items-center justify-end mt-4 gap-4">
                            <a href="{{ route('notices.index') }}" class="text-sm text-gray-400 dark:text-gray-400 hover:underline">취소</a>
                            <x-primary-button>
