        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-100">
                    <form method="POST" action="{{ route('doc_lists.update', $docList->idx ?? $docList->id) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method("PUT")
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="mt-1 block w-full bg-gray-900 border-gray-700 text-white rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($types as $t)
                                    <option value="{{ $t->idx }}" {{ old('type', $docList->type) == $t->idx ? 'selected' : '' }}>
                                        {{ $t->mtype }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            @if($docList->filename)
                                <div class="mb-2 text-xs text-gray-400">
                                    현재 파일: <a href="{{ asset($docList->path . '/' . $docList->filename) }}" target="_blank" class="text-blue-400 hover:underline">{{ $docList->filename }}</a>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <select id="language" name="language" class="mt-1 block w-full bg-gray-900 border-gray-700 text-white rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($languages as $lang)
                                    <option value="{{ $lang->idx }}" {{ old('language', $docList->language) == $lang->idx ? 'selected' : '' }}>
                                        {{ $lang->mlanguage }}
                                    </option>
                                @endforeach
                            </select>
